...
import {
  getTendenciaPorGrupo,
  getTendenciaPorSubGrupo,
  getTopExcedenteAnual,
  getTopExcedenteMensal,
} from "./tendencia.service";
import { ExcedenteAno, TendenciaPorGrupo } from "./tendencia.types";

const USE_MOCK = false;

export const fetchTendenciaPorGrupo = async (
  grupoId: string,
): Promise<TendenciaPorGrupo[]> => {
...
export const fetchTendenciaPorSubgrupoGrupo = async (
  grupoId: string,
  subgrupoId: string,
): Promise<TendenciaPorGrupo[]> => {
  let data: TendenciaPorGrupo[];
...
export const fetchExcedenteAno = async (
  ano: number,
): Promise<ExcedenteAno[]> => {
...
export const fetchExcedenteMensal = async (
  ano: number,
  mes: number
): Promise<ExcedenteAno[]> => {
  let data: ExcedenteAno[];
  if (USE_MOCK) {
    ...
    const response = await getTopExcedenteMensal(ano, mes);
    data = response.data ?? [];
  }
  return data;
};